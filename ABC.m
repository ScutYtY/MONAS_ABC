classdef Two_Arch2ABC < ALGORITHM
    ...
    methods
        function main(Algorithm,Problem)
            ...
            while Algorithm.NotTerminated(DA)
                ...
                EOffspring  = OperatorEmployedABC(ParentC,Problem.D,Problem.N,Problem.M,Problem); 
                %Offspring         = [OperatorGA(Problem,ParentC,{1,20,0,0}),OperatorGA(Problem,ParentM,{0,0,1,20})];
                
                OOffspring  = OperatorOnlookerABC(CA,Problem.D,Problem.N,Problem.M,Problem);
                CA = UpdateCA(CA,Offspring,CAsize);
                DA = UpdateDA(DA,OOffspring,Problem.N,p);
            end
        end
    end
end